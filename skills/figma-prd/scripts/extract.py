...
- **변경/추가 표시**: "변경"·"추가" 라벨 TEXT 노드의 주석 박스 영역(bbox)을 찾아,
  그 안에 들어가는 콘텐츠 텍스트에 ``[변경]``/``[추가]`` 태그를 단다. 색/fill 판정이
  아니라 라벨 텍스트 일치 + geometry 겹침이라 결정적이다.
...
# 변경/추가 주석 박스 라벨. config의 ``change_labels`` 로 override 가능.
CHANGE_LABELS = {"변경", "추가"}
...
def collect_change_regions(
    ...
    """'변경'/'추가' 라벨 TEXT 노드 → 주석 박스 highlight 영역(bbox) 리스트.
...
def walk(
    ...
    if node_type == "TEXT":
        ...
        # "변경"/"추가" 라벨은 주석 마커지 콘텐츠가 아니므로 본문 texts에서 제외.

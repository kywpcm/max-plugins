...
├── README.md                             # 이 파일
├── scripts/
│   ├── extract.py                        # Figma REST API → texts.md/images/screenshot
...
1. `extract.py` — REST API로 노드 트리·텍스트·이미지 수집 (결정적)
2. Agent 서브에이전트 — 노드별 모드별 멀티모달 분석 (병렬 가능)
3. `synthesize.py` — 텍스트 + 분석 결과 → PRD 마크다운 합성

...
The goal is to create a readable record of what `User(사용자)` and Claude discussed and accomplished — not a verbatim transcript, but a structured summary that's useful for future reference.
...
User(사용자): {One sentence summary of request}
...
- `User(사용자):` lines capture the user's intent in one sentence (not verbatim)
- `Claude:` lines describe what was actually done — use bullet lists when there are multiple actions or results
- Do NOT use bold markup (`**`) on User(사용자)/Claude names — keep them plain text
